...
from q_scope.implementations.datastrutures import (
    ...
    RefreshToken, AccessToken, AuditLog
)
...
class AuthenticateClientCondition(Condition):
    ...
    async def validate(self, *, context: Mapping[str, Any], ray_id: str) -> Result:
        ...
        client_result = await self._storage.clients.get_by_client_identifier(client_id, ray_id)
...
class CheckAccessTokenLimitCondition(Condition):
    ...
    async def validate(self, *, context: Mapping[str, Any], ray_id: str) -> Result:
        ...
        if count >= limit:
             ...
             if oldest:
                 oldest.revoked_at = 1 # Mark as revoked
                 await self._storage.access_tokens.update(oldest, ray_id)
...
class RefreshTokenFlow(OAuth2Authorization):
    ...
    async def _run(self, context: Mapping[str, Any], ray_id: str) -> Mapping[str, Any]:
        ...
        original_scopes = set(refresh_token_obj.scopes.split()) if refresh_token_obj.scopes else set()
        
        if requested_scope:
            requested_scopes_set = set(requested_scope.split())
            if not requested_scopes_set.issubset(original_scopes):
                raise OAuthException(OAuthErrors.INVALID_SCOPE, "Requested scope includes scopes not originally granted")
            final_scope = requested_scope
        else:
            final_scope = refresh_token_obj.scopes
    ...
    async def _postconditions(self, context: Mapping[str, Any], result: Mapping[str, Any], ray_id: str) -> None:
        ...
        if result["is_rotated"]:
             # Revoke old
             refresh_token_obj.revoked_at = timestamp
             await self._storage.refresh_tokens.update(refresh_token_obj, ray_id)
             ...
             refresh_token_obj.updated_at = timestamp
             await self._storage.refresh_tokens.update(refresh_token_obj, ray_id)
        ...
        audit_log = AuditLog(
            ...
            subject=f"refresh_token:{refresh_token_obj.id}",
            client_id=client.id,
            user_id=refresh_token_obj.user_id,
            metadata=f"rotated={result['is_rotated']}",
            created_at=timestamp,
            created_by="system",
            updated_at=timestamp,
            updated_by="system"
        )
        await self._storage.audit_logs.insert(audit_log, ray_id)
